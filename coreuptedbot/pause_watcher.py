...
class pause_watcher:
      ...
      def watchforpause(self, pausestatus):
        ...
        for message in message_list:
            ...
            if chat == "!pausebot":
                if chatter.isadmin() or chatter.isowner():
                    self.chat_client.chat("Coreuptedbot is paused! That means {} and announcements are paused. !unpausebot to unpause".format(POINT_NAME))
                    return True
                else:
                    self.chat_client.chat("Only admins and the owner may use !pausebot") 
                    return pausestatus
            if chat == "!unpausebot":
                if chatter.isadmin() or chatter.isowner():
                    self.chat_client.chat("Coreuptedbot is unpaused! Come and get your {}".format(POINT_NAME))
                    return False
                else:
                    self.chat_client.chat("Only admins and the owner may use !pausebot") 
                    return pausestatus
